...
const OrgChart = ({ 
    ...
    // Flatten data for list view and filtering
    const flatEmployees = useMemo(() => {
        const list = [];
        const walk = (nodes) => {
            nodes.forEach(node => {
                list.push({
                    id: node.employeeNo || node.id,
                    name: node.employeeName || node.name,
                    designation: node.designation,
                    department: node.department,
                    status: (node.status || "Active").toLowerCase() === "active" ? "Active" : "Inactive",
                    reportingManager: node.reportingManager,
                    role: node.role,
                    avatar: node.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(node.employeeName || node.name)}`,
    ...
    const filteredEmployees = useMemo(() => {
        if (filterDept === 'All') return flatEmployees;
        return flatEmployees.filter(e => e.department === filterDept);
    }, [flatEmployees, filterDept]);

    const departments = useMemo(() => {
        return ['All', ...new Set(flatEmployees.map(e => e.department))].filter(Boolean);
    }, [flatEmployees]);
    ...
    const hierarchy = useMemo(() => {
        ...
        return {
            ...
            department: "Global",
            status: "Active",
            avatar: "https://ui-avatars.com/api/?name=ORG&background=1e3a8a&color=fff",
            children: roots,
        };
    }, [data]);
    ...
    if (loading) {
        return <div className="p-10 text-lg flex items-center justify-center h-64">
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100 rounded-[2rem] border border-blue-100 shadow-[0_20px_50px_rgba(0,0,0,0.05)] relative">
            ...
            <div className="h-20 border-b flex items-center justify-between px-8 bg-white/90 backdrop-blur-xl z-20 border-blue-50 shrink-0">
                <div className="flex items-center gap-6">
                    <div className={showCount ? "pt-6" : ""}>
                        <h1 className="text-[24px] font-bold bg-gradient-to-r from-blue-700 to-indigo-600 bg-clip-text text-transparent tracking-tight">{title}</h1>
                    </div>
                    <div className="h-8 w-px bg-slate-200 mx-2 hidden md:block mt-2"></div>
                </div>
            ...
            {selectedEmployee && (
                <>
                    <div onClick={() => setSelectedEmployee(null)} className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[999] animate-in fade-in duration-500" />
                            ...
                            {activeTab === 'details' && (
                                <div className="space-y-5">
                                    ...
                                    {[
                                        ...
                                        { label: "Work Email", value: selectedEmployee.companyEmail || "N/A" },
                                        { label: "Contact No", value: selectedEmployee.phone || "N/A" },
                                        { label: "Joining Date", value: selectedEmployee.joiningDate || "N/A" },
                                    ].map((item, i) => (
                                        <div
                                            key={i}
