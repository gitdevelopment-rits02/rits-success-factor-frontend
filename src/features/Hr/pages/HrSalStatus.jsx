...
const employees = [
  { id:1,  name:"Aarav Mehta",   role:"Sr. Developer",   dept:"Engineering", emp:"EMP-2024-0011", ctc:1140000, pf:7600,  tds:14200, pension:9500  },
  { id:2,  name:"Priya Sharma",  role:"HR Manager",       dept:"HR",          emp:"EMP-2024-0022", ctc:864000,  pf:5760,  tds:9800,  pension:7200  },
  { id:3,  name:"Rohan Gupta",   role:"Analyst",          dept:"Finance",     emp:"EMP-2024-0033", ctc:816000,  pf:5440,  tds:8600,  pension:6800  },
  { id:4,  name:"Sneha Patel",   role:"Frontend Dev",     dept:"Engineering", emp:"EMP-2024-0044", ctc:1020000, pf:6800,  tds:12400, pension:8500  },
  { id:5,  name:"Vikram Nair",   role:"Sales Lead",       dept:"Sales",       emp:"EMP-2024-0055", ctc:936000,  pf:6240,  tds:10200, pension:7800  },
  { id:6,  name:"Divya Iyer",    role:"Brand Manager",    dept:"Marketing",   emp:"EMP-2024-0066", ctc:888000,  pf:5920,  tds:9400,  pension:7400  },
  { id:7,  name:"Karan Joshi",   role:"Backend Dev",      dept:"Engineering", emp:"EMP-2024-0077", ctc:1080000, pf:7200,  tds:13600, pension:9000  },
  { id:8,  name:"Meera Rao",     role:"CFO",              dept:"Finance",     emp:"EMP-2024-0088", ctc:1560000, pf:10400, tds:24800, pension:13000 },
  { id:9,  name:"Arjun Singh",   role:"Recruiter",        dept:"HR",          emp:"EMP-2024-0099", ctc:744000,  pf:4960,  tds:7200,  pension:6200  },
  { id:10, name:"Pooja Desai",   role:"Account Exec",     dept:"Sales",       emp:"EMP-2024-0101", ctc:852000,  pf:5680,  tds:9000,  pension:7100  },
  { id:11, name:"Nikhil Kumar",  role:"SEO Specialist",   dept:"Marketing",   emp:"EMP-2024-0112", ctc:720000,  pf:4800,  tds:6800,  pension:6000  },
  { id:12, name:"Ananya Bose",   role:"QA Engineer",      dept:"Engineering", emp:"EMP-2024-0123", ctc:948000,  pf:6320,  tds:10600, pension:7900  },
];

// Pre-seeded dummy data: months 0–1 (Jan–Feb) have realistic statuses and txn IDs.
// Month 2 (Mar) and beyond default to "pending".
const SEED_DATA = {
  // Jan (0)
  "0": {
    statuses: { 1:"credited",2:"credited",3:"credited",4:"credited",5:"credited",6:"credited",7:"credited",8:"credited",9:"credited",10:"credited",11:"credited",12:"credited" },
    txnIds:   { 1:"TXN-2025-JAN-00112",2:"TXN-2025-JAN-00113",3:"TXN-2025-JAN-00114",4:"TXN-2025-JAN-00115",5:"TXN-2025-JAN-00116",6:"TXN-2025-JAN-00117",7:"TXN-2025-JAN-00118",8:"TXN-2025-JAN-00119",9:"TXN-2025-JAN-00120",10:"TXN-2025-JAN-00121",11:"TXN-2025-JAN-00122",12:"TXN-2025-JAN-00123" },
  },
  // Feb (1)
  "1": {
    statuses: { 1:"credited",2:"credited",3:"hold",4:"credited",5:"in_transaction",6:"credited",7:"credited",8:"hold",9:"credited",10:"credited",11:"in_transaction",12:"credited" },
    txnIds:   { 1:"TXN-2025-FEB-00201",2:"TXN-2025-FEB-00202",4:"TXN-2025-FEB-00204",6:"TXN-2025-FEB-00206",7:"TXN-2025-FEB-00207",9:"TXN-2025-FEB-00209",10:"TXN-2025-FEB-00210",12:"TXN-2025-FEB-00212" },
  },
};
...
// Returns status for a given employee + month, checking saved overrides first,
// then seed data, then defaulting to "pending".
function getStatus(monthOverrides, moKey, empId) {
  if (monthOverrides[moKey]?.[empId]) return monthOverrides[moKey][empId];
  return SEED_DATA[moKey]?.statuses?.[empId] || "pending";
}

function getTxnId(monthTxns, moKey, empId) {
  if (monthTxns[moKey]?.[empId]) return monthTxns[moKey][empId];
  return SEED_DATA[moKey]?.txnIds?.[empId] || "";
}

// ── CARD ──
function EmpCard({ emp, status, onClick }) {
  const av = avatarColors[(emp.id - 1) % avatarColors.length];
  const statusMap = {
    credited:       { dot:"#34a853", label:"Paid" },
    hold:           { dot:"#f9ab00", label:"On Hold" },
    in_transaction: { dot:"#9334e6", label:"Processing" },
    pending:        { dot:"#94a3b8", label:"Pending" },
  };
  const { dot, label } = statusMap[status] || statusMap.pending;

  return (
    <div onClick={onClick} style={{
      background:"#fff", borderRadius:16, padding:"20px",
      border:"1.5px solid #e8f0fe", cursor:"pointer", transition:"all .18s", position:"relative",
    }}
    ...
    >
      ...
      <div style={{
        width:54,height:54,borderRadius:14,background:av.bg,color:av.color,
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          ...
          <span style={{fontSize:11,color:dot,fontWeight:600}}>{label}</span>
        </div>
      </div>
    </div>
  );
...
// ── DETAIL ──
function EmpDetail({ emp, mo, moKey, getStatusFn, getTxnFn, onSaveStatus, onBack }) {
  const av  = avatarColors[(emp.id - 1) % avatarColors.length];
  const c   = calc(emp);
  const cur = getStatusFn(moKey, emp.id);
  const savedTxn = getTxnFn(moKey, emp.id);

  const [txnInput, setTxnInput]           = useState(savedTxn);
  const [pendingStatus, setPendingStatus] = useState(cur);
  const [saved, setSaved]                 = useState(false);

  const statusColor = { credited:"#34a853", hold:"#f9ab00", in_transaction:"#9334e6", pending:"#94a3b8" }[cur] || "#94a3b8";
  const statusLabel = { credited:"salary credited ✓", hold:"salary on hold ⏸", in_transaction:"in transaction 🔄", pending:"pending — not yet processed" }[cur] || "pending";

  const handleSave = () => {
    onSaveStatus(moKey, emp.id, pendingStatus, txnInput.trim());
    setSaved(true);
    setTimeout(() => setSaved(false), 2500);
  };

  const hasChanges = pendingStatus !== cur || txnInput.trim() !== savedTxn;

  return (
    <div style={{maxWidth:700,margin:"0 auto"}}>
      <button onClick={onBack} style={{
        display:"flex",alignItems:"center",gap:6,background:"none",border:"none",
        cursor:"pointer",color:"#64748b",fontSize:13,fontWeight:500,marginBottom:22,padding:0,
      }}>
      ...
      <div style={{background:"#fff",borderRadius:16,padding:"22px 24px",border:"1.5px solid #e8f0fe",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
          <div style={{
            width:62,height:62,borderRadius:16,background:av.bg,color:av.color,
          ...
          <div style={{flex:1,minWidth:160}}>
            ...
            <div style={{marginTop:8}}>
              <span style={{background:"#e8f0fe",color:"#4f6ef7",fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:99}}>{emp.emp}</span>
            </div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{fontSize:10.5,color:"#94a3b8",fontWeight:600,textTransform:"uppercase",letterSpacing:".6px"}}>Net Monthly Pay</div>
            <div style={{fontSize:30,fontWeight:800,color:"#2563eb",letterSpacing:"-1px",marginTop:2}}>{f(c.net)}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        <div style={{background:"#fff",borderRadius:16,padding:"20px 22px",border:"1.5px solid #e8f0fe"}}>
          ...
          {[{l:"Basic Pay",v:c.basic},{l:"House Rent Allowance (HRA)",v:c.hra},{l:"Special Allowance",v:c.special},{l:"Conveyance Allowance",v:c.conv}].map(r=>(
        ...
        <div style={{background:"#fff",borderRadius:16,padding:"20px 22px",border:"1.5px solid #e8f0fe"}}>
          ...
          {[{l:"Professional Tax",v:c.pt},{l:"Provident Fund (PF)",v:emp.pf},{l:"Income Tax (TDS)",v:emp.tds}].map(r=>(
            <div key={r.l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #f8faff"}}>
              <span style={{fontSize:13,color:"#475569"}}>{r.l}</span>
              <span style={{fontSize:13,fontWeight:600,color:"#0f172a"}}>{f(r.v)}</span>
            </div>
      ...
      <div style={{background:"#f0f7ff",borderRadius:16,padding:"22px 24px",border:"1.5px solid #dbeafe"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:20}}>
          {[
            { key:"pending",        label:"Pending",      icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
            { key:"credited",       label:"Credited",     icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg> },
            { key:"hold",           label:"On Hold",      icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> },
            { key:"in_transaction", label:"Transaction",  icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg> },
          ].map(btn=>{
            const isActive = pendingStatus === btn.key;
            return (
              <button key={btn.key} onClick={()=>setPendingStatus(btn.key)} style={{
                ...
                cursor:"pointer", transition:"all .15s", fontWeight:isActive?700:500, fontSize:12.5,
        ...
        {/* Transaction ID row */}
        <div style={{background:"#fff",border:"1.5px solid #dbeafe",borderRadius:14,padding:"16px 18px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",marginBottom:14}}>
          <div style={{width:36,height:36,borderRadius:10,background:"#eff6ff",border:"1.5px solid #bfdbfe",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          ...
          <div style={{flex:1,minWidth:180}}>
            <div style={{fontSize:11,fontWeight:600,color:"#64748b",textTransform:"uppercase",letterSpacing:".5px",marginBottom:5}}>Transaction ID</div>
            <input value={txnInput} onChange={e=>{setTxnInput(e.target.value);setSaved(false);}}
              placeholder="e.g. TXN-2025-0348291"
              style={{width:"100%",border:"1.5px solid #dbeafe",borderRadius:9,padding:"9px 12px",fontSize:13,color:"#0f172a",background:"#f8fbff",fontFamily:"'Plus Jakarta Sans','Helvetica Neue',sans-serif",transition:"border-color .15s",outline:"none"}}
          ...
          <button onClick={handleSave} disabled={!hasChanges&&!saved} style={{
            ...
            border:"none",borderRadius:11,padding:"0 22px",height:42,fontSize:13,fontWeight:700,
            ...
            {saved ? (
              ...
              <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>Save Status</>
            )}
          </button>
        </div>

        {savedTxn && (
          <div style={{display:"flex",alignItems:"center",gap:8,background:"#f0fdf4",border:"1px solid #d1fae5",borderRadius:9,padding:"9px 14px",marginBottom:12}}>
            ...
            <span style={{fontSize:12,color:"#166534"}}>Last saved Transaction ID: <strong style={{letterSpacing:".3px"}}>{savedTxn}</strong></span>
          </div>
        )}

        {/* nudge messages */}
        {cur==="pending" && (
          <div style={{marginTop:4,background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <span style={{fontSize:12.5,color:"#64748b"}}>salary not yet processed for {emp.name.split(" ")[0]} — initiate payment for {FULL[mo]}</span>
            <button onClick={()=>setPendingStatus("in_transaction")} style={{background:"#2563eb",color:"#fff",border:"none",borderRadius:8,padding:"7px 14px",fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>
              initiate →
            </button>
          </div>
        )}
...
export default function App() {
  ...
  // monthOverrides[moKey][empId] = status — only user-made changes live here
  const [monthOverrides, setMonthOverrides] = useState({});
  // monthTxns[moKey][empId] = txnId
  const [monthTxns, setMonthTxns] = useState({});

  const moKey = String(mo);

  const getStatusFn = (mk, empId) => getStatus(monthOverrides, mk, empId);
  const getTxnFn    = (mk, empId) => getTxnId(monthTxns, mk, empId);

  const saveStatus = (mk, empId, status, txn) => {
    setMonthOverrides(p => ({ ...p, [mk]: { ...p[mk], [empId]: status } }));
    if (txn) setMonthTxns(p => ({ ...p, [mk]: { ...p[mk], [empId]: txn } }));
  };
  ...
  const counts = employees.reduce((acc, e) => {
    const s = getStatusFn(moKey, e.id);
    acc[s] = (acc[s] || 0) + 1;
    return acc;
  }, {});
    ...
    <div style={{minHeight:"100vh",background:"#f0f7ff",fontFamily:"'Plus Jakarta Sans','Helvetica Neue',sans-serif"}}>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #dbeafe",padding:"12px 32px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,position:"sticky",top:0,zIndex:10}}>
        ...
        <div style={{display:"flex",gap:2,background:"#f0f7ff",border:"1px solid #dbeafe",borderRadius:10,padding:"3px"}}>
          {MONTHS.map((m,i)=>(
            <button key={m} onClick={()=>{ setMo(i); setSel(null); }} style={{
        ...
        {!sel ? (
          <>
            ...
            <div style={{display:"flex",gap:12,marginBottom:20,flexWrap:"wrap"}}>
              {[
                {label:"Total",     count:employees.length,        color:"#2563eb", bg:"#eff6ff"},
                {label:"Credited",  count:counts.credited||0,      color:"#34a853", bg:"#f0fdf4"},
                {label:"Pending",   count:counts.pending||0,       color:"#64748b", bg:"#f8fafc"},
                {label:"On Hold",   count:counts.hold||0,          color:"#f9ab00", bg:"#fffbeb"},
                {label:"Processing",count:counts.in_transaction||0,color:"#9334e6", bg:"#f5f3ff"},
              ].map(s=>(
                <div key={s.label} style={{background:s.bg,border:`1.5px solid ${s.color}22`,borderRadius:12,padding:"10px 18px",display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:22,fontWeight:800,color:s.color,lineHeight:1}}>{s.count}</span>
                  <span style={{fontSize:11.5,color:s.color,fontWeight:600,opacity:.8}}>{s.label}</span>
                </div>
              ))}
            </div>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {depts.map(d=>(
                  <button key={d} onClick={()=>setDept(d)} style={{
                    background:dept===d?"#2563eb":"#fff", color:dept===d?"#fff":"#64748b",
                    border:`1.5px solid ${dept===d?"#2563eb":"#dbeafe"}`,
                    borderRadius:99,padding:"6px 14px",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all .12s",
                  }}>{d}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:16}}>
              {list.map(emp=>(
                <EmpCard key={emp.id} emp={emp} status={getStatusFn(moKey, emp.id)} onClick={()=>setSel(emp)}/>
              ))}
              {list.length===0 && (
