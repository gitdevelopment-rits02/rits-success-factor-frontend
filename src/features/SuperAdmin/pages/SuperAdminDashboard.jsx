        ...
        <>
          ...
          <div className="space-y-2">
            {Object.entries(deptCounts).map(([dept, count]) => (
              <div key={dept} className="flex justify-between text-sm font-bold">
...
function ImgStatCard({ label, value, icon: Icon, color }) {
  return (
    <div className="bg-white p-8 rounded-[1.5rem] border border-slate-100 shadow-sm flex justify-between items-start group hover:border-blue-200 transition-colors">
      <div>
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{label}</p>
        <p className="text-4xl font-black text-slate-800">{value}</p>
      </div>
      <Icon className={`text-2xl ${color} opacity-80`} />
    </div>
  );
}