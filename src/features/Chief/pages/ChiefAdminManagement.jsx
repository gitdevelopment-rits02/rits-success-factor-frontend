...
const ChiefAdminManagement = () => {
    const [admins, setAdmins] = useState(() => {
        const saved = localStorage.getItem('successfactor_chief_admins');
    ...
    // Load superadmin (chief) to set as reporting manager
    const chief = useMemo(() => {
        const saved = localStorage.getItem('successfactor_chief');
        return saved ? JSON.parse(saved) : { name: "Chief Admin" };
    }, []);
    ...
    React.useEffect(() => {
        localStorage.setItem('successfactor_chief_admins', JSON.stringify(admins));
    }, [admins]);
    ...
    const handleCreate = () => {
         ...
         * Logic: We assign every newly created Admin/HR to report to the Chief.
        ...
        setAdmins(prev => [
            ...prev,
            {
                ...
                reportingManager: chief.name, // Link to the Chief
...
export default ChiefAdminManagement;
